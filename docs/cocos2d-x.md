...
Encontramos también una serie de tipos de datos geométricos: `Vec2`, `Point`, `Rect` y `Size`. Estos tipos de datos incorporan también algunas
...
Todos los nodos de Cocos2d-x comparten una serie de propiedades comunes, a las que podremos acceder con los correspondientes métodos `get`/`set`. En la siguiente figura se muestran las principales propiedades:

![Propiedades de los nodos](imagenes/cocos/cocos_propiedades.png)

Las propiedades `position` y `anchorPoint` nos permiten posicionar un objeto a partir de sus coordenadas 2D y de su punto de anclado (tipo `Vec2`):

```cpp
node->setAnchorPoint(Vec2(0.5, 0.5));
node->setPosition(Vec2(240, 160));
```

Podemos modificar también su rotación y escala:

```cpp
node->setRotation(M_PI * 0.5);
node->setScale(0.5f);
```

Podemos consultar el área que ocupa el nodo en pantalla con:

```cpp
Size s = node->getContentSize();
```

Podemos indicar si el nodo es visible (en caso de no serlo no se renderizará, reduciendo así el coste del _render_), o controlar su opacidad:
...
Otra propiedad importante en los nodos es su _orden Z_. En caso de Cococs2d-x se llama `localZOrder` para hacer énfasis en que el orden afecta sólo de forma local entre nodos hermanos (hijos de un mismo nodo):
...
Vamos a ver las formas en las que podemos actualizar el estado de la escena en cada iteración del ciclo del juego. Podremos hacerlo o bien indicando en un método `update` el código a ejecutar en cada iteración, o bien programando las actualizaciones mediante _acciones_.


### Actualización en el ciclo del juego

En Cocos2d-x no deberemos preocuparnos de implementar el ciclo del juego, ya que de esto se encarga el _singleton_ `Director`. Éste objeto será el encargado de actualizar y dibujar la escena actual

La escena tiene un método `schedule` que permite especificar un método al que se llamará en cada iteración del ciclo. De esa forma, podremos especificar en dicho método la forma de actualizar la escena:
...
que se encargará de actualizar la escena. Como parámetro recibe el tiempo transcurrido desde la anterior actualización (_delta time_). Deberemos aprovechar este tiempo para actualizar los movimientos a partir de él, y así conseguir un movimiento fluido y constante:
...
En este caso estamos moviendo el _sprite_ en _x_ a una velocidad de 100 pixeles por segundo (el tiempo transcurrido se proporciona en segundos). 

> Es importante remarcar que tanto el dibujado como las actualizaciones sólo se llevarán a cabo cuando la escena en la que están sea la escena activa en el `Director`. 
...
El método `schedule` nos permite indicar también la periodicidad con la que se llamará a la función indicada. Por defecto será en cada iteración del ciclo del juego (normalmente _60 fps_), pero podemos utilizar periodicidades distintas. Por ejemplo, si lo único que necesitamos es actualizar un temporizador que mide el tiempo en segundos, podríamos llamar a la función una vez por segundo.


### Acciones

En el punto anterior hemos visto cómo actualizar la escena de forma manual, como se hace habitualmente en el ciclo del juego. Sin embargo, con Cocos2d-x tenemos formas más sencillas de animar los nodos de 
la escena, son lo que se conoce como **acciones**. 

Estas acciones nos permiten definir determinados comportamientos, como trasladarse a un determinado punto, y aplicarlos sobre un nodo para que realice dicha acción de forma automática, sin tener que actualizar su posición manualmente en cada iteración (_tick_) del juego.

Todas las acciones derivan de la clase `Action`. Encontramos acciones instantáneas (como por ejemplo situar un nodo en una posición determinada), o acciones con una duración (mover el nodo hasta la posición destino gradualmente).

#### Ejecución de acciones

Por ejemplo, para mover un nodo a la posición _(200, 50)_ en 3 segundos, podemos definir una  acción como la siguiente:
...
#### Combinación y repetición de acciones

Además, tenemos la posibilidad de encadenar varias acciones mediante el tipo especial de acción `Sequence`. En el siguiente ejemplo primero situamos el nodo de forma inmediata en _(0, 50)_, y después lo movermos a _(200, 50)_:
...
nodo->runAction(actionRepeat);
```

De esta forma, el nodo estará continuamente moviéndose de _(0,50)_ a _(200,50)_. Cuando llegue a la posición final volverá a aparecer en la inicial y continuará la animación.

#### Llamadas a funciones propias

Encontramos también acciones que nos permiten realizar tareas personalizadas, proporcionando mediante una pareja _target-selector_ la función a la que queremos que se llame cuando se produzca la acción:
...
También encontramos variantes de esta acción que nos permiten pasarle al _callback_ como parámetro datos propios o el nodo sobre el que se ha ejecutado la acción (`CallFuncN` recibe el nodo como parámetro, y `CallFundND` recibe el nodo y un puntero a datos genéricos). Cuanto tengamos que pasar un _callback_ con parámetros utilizaremos `CC_CALLBACK_1`, `CC_CALLBACK_2` y `CC_CALLBACK_3`, para 1, 2 y 3 parámetros respectivamente.
...
El _sprite_ podrá ser añadido a la escena como cualquier otro nodo, añadiéndolo como hijo de alguna de las capas con `addChild()`.
...
Por ejemplo, para posicionar un _sprite_ en unas determinadas coordenadas le asignaremos un valor a su propiedad `position` (esto es aplicable a cualquier nodo):
...
podríamos modificar esto con la propiedad `anchorPoint`, de forma similar a las capas de _Core Animation_. El sistema de coordenadas de Cocos2D es el mismo que el de _Core Graphics_, el origen de coordenadas se encuentra en la esquina inferior izquierda, y las _y_ son positivas hacia arriba.
...
Recordamos que todas estas propiedades no son exclusivas de los _sprites_, sino que son aplicables a cualquier nodo, aunque tienen un especial interés en el caso de los _sprites_.
...
Estos objetos pueden estar animados. Para ello deberemos definir los distintos fotogramas (o _frames_) de la animación. Podemos definir varias animaciones para cada _sprite_, según las acciones que pueda hacer, y cada animación se definirá como una secuencia de fotogramas. 

Por ejemplo, si tenemos un personaje podemos tener una animación para  andar hacia la derecha y otra para andar hacia la izquierda.

Para almacenar los fotogramas de los _sprites_ de forma óptima, utilizamos lo que se conoce como _sprite sheets_. Se trata de imágenes en las que incluyen de forma conjunta todos los fotogramas de los _sprites_, organizándolos de forma óptima para aprovechar el espacio de la imagen .

Estos _sprite sheets_ pueden crearse con herramientas como [**TexturePacker**](http://www.texturepacker.com), [**Shoebox**](https://renderhjs.net/shoebox/) o [**FreeTexturePacker**](http://free-tex-packer.com). Todas estas herramientas exportan el _sprite sheet_ al formato reconocido por Cocos2d-x. El formato que podremos leer con este motor cuenta con dos ficheros:

* `.png`: Textura del _sprite sheet_, donde se encuentran todos los fotogramas organizados de forma óptima en el espacio de la imagen.
* `.plist`: Índice del _sprite sheet_, donde se indica la lista de fotogramas que contiene (cada uno se identifica por un nombre en el índice), y para cada uno se define la región que ocupa en la textura, para que así el motor sea capaz de extraer cada fotograma por separadio.


En Cocos2d-x tenemos la clase `SpriteFrameCache` que se encarga de almacenar la caché de fotogramas de _sprites_ que queramos utilizar. 
Podremos añadir los fotogramas de nuestro _sprite sheet_ a la caché a partir de los dos ficheros indicados anteriormente (sólo hará falta hacer referencia al fichero `.plist`, ya que este fichero incluirá una referencia al `.png`):
...
Vemos que la caché de fotogramas se define como _singleton_, y podremos acceder a ella para obtener los fotogramas que necesitemos en cada momento.

Una vez introducidos los fotogramas del _sprite sheet_ en la caché de fotogramas, podemos crear _sprites_ a partir de dicha caché con:

```cpp
Sprite *sprite = Sprite::createWithSpriteFrameName("frame01.png");
```

En el caso anterior creamos un nuevo _sprite_, pero en lugar de hacerlo directamente a partir de una imagen, lo hacemos a partir del nombre de un fotograma añadido a la caché de textura. 

> No debemos confundirnos con el nombre que se le proporciona como parámetro, parece que sea un fichero de imagen, pero no lo es. En el índice `.plist` por defecto a cada fotograma se le da como nombre identificativo el nombre del fichero de la imagen original, y por eso al hacer referencia a él indicamos dicho nombre de fichero, pero **no está accediendo a un fichero**, sino a la caché de fotogramas. 


También podemos obtener el fotograma como un objeto `SpriteFrame`. Esta clase no define un _sprite_, sino el fotograma almacenado en caché. Es decir, no es un nodo que podamos almacenar en la 
...
Podremos inicializar también el _sprite_ a partir del fotograma anterior, en lugar de hacerlo directamente a partir del nombre del fotograma:
...
Podremos definir determinadas secuencias de _frames_ para crear animaciones. Las animaciones se representan mediante la clase `Animation`, y se pueden crear a partir de la secuencia de
fotogramas que las definen. Los fotogramas deberán indicarse mediante objetos de la clase  `SpriteFrame`:
...
Una vez definida la animación, podemos añadirla a una caché de animaciones que, al igual que la caché de texturas, también se define como _singleton_:
...
La animación se identifica mediante la cadena que proporcionamos como parámetro `name`. Podemos cambiar el fotograma que muestra actualmente un _sprite_ con su método:

```cpp
sprite->setDisplayFrameWithAnimationName("animAndar", 0);
```

Con esto buscará en la caché de animaciones la animación especificada, y mostrará de ella el fotograma cuyo índice proporcionemos. Pero lo realmente interesante será poder reproducir la animación de forma automática. Esto podremos hacerlo mediante acciones.

### La acción `Animate`

Podemos aprovechar este mecanismo de acciones para definir las animaciones de fotogramas de los _sprites_, con una acción de tipo `Animate`. Crearemos la acción de animación a partir de una animación de la caché de animaciones:
...
Con esto estaremos reproduciendo continuamente la secuencia de fotogramas definida en la animación, utilizando la periodicidad (_delayPerUnit_) que especificamos al crear dicha animación.


### _Sprite batch_

En OpenGL los _sprites_ se dibujan realmente en un contexto 3D. Es decir, son texturas que se  mapean sobre polígonos 3D (concretamente con una geometría rectángular). Muchas veces encontramos en
...
Podemos optimizar el dibujado de _sprites_ cuyos fotogramas se encuentren dentro de una misma texture utilizando un _sprite batch_. Creamos un _batch_ de _sprites_ con Cocos2d-x utilizando la clase `SpriteBatchNode`:
...
El _sprite batch_ es un tipo de nodo más que podemos añadir a nuestra escena, pero por si sólo no genera ningún contenido. Deberemos añadir como hijos los _sprites_ que queremos que dibuje. Es imprescindible que los hijos sean de tipo `Sprite` (o subclases de ésta), y que tengan como textura la misma textura que hemos utilizado al crear el _batch_ (o regiones de la misma). No podremos añadir _sprites_ con ninguna otra textura dentro de este _batch_.
...
> En el ejemplo anterior consideramos que el _frame_ con nombre `"frame01.png"` es un fotograma que se cargó en la caché de fotogramas a partir de la textura `sheet.png`. De no pertenecer a dicha textura no podría cargarse dentro del _batch_.





### Colisiones y _bounding boxes_
...
Con esto podremos saber si un enemigo nos ha tocado, o si una de nuestras balas ha impactado en un enemigo.
